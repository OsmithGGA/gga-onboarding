    ...
    <div className="space-y-6">
      {completed && (
        <div className="flex items-center gap-2 p-3 bg-[#ADFF00]/5 border border-[#ADFF00]/20 rounded-xl">
          ...
          <p className="text-[#ADFF00] text-xs font-medium">Step complete — your call is booked. Calendar below if you need to reschedule.</p>
        </div>
      )}
      {/* Info banner */}
      ...
      {!completed && (
        !choice ? (
          <div className="flex flex-col sm:flex-row gap-3">
            <button
              onClick={() => { setChoice("book"); onComplete("Booked via portal"); }}
              disabled={completing}
            ...
            <button
              onClick={() => { setChoice("already"); onComplete("Already booked before portal"); }}
              disabled={completing}
